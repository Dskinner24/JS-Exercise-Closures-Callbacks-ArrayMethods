...
function lowerCaseStrings(strings){
  let fruits = [];
  strings.forEach(fruit => fruits.push(fruit.toLowerCase()))
  return fruits
}
...
let checkArray = ['apple'];
let newArray = strings.map(n => checkArray.includes(n));
return newArray
}
...
function stringSmash(strings) {
  let stringArray = (accum, currentValue) => accum + currentValue;
  return strings.reduce(stringArray);
}
...
function getFullNames(runners) {
  let newArray = runners.map((names) => `${names.last_name}, ${names.first_name}`)
  return newArray
}
...
function firstNamesAllCaps(runners) {
  let newArray = runners.map((names) => `${names.first_name.toUpperCase()}`)
  return newArray
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
 let newArray = runners.filter(shirts => shirts.shirt_size === "2XL")
  return(newArray)
}
 ...
 * 
 * the count variable in counter1 is in the local scope of the function whereas the count variable in counter 2 is in the global scope
 * 
 * 2. Which of the two uses a closure? How can you tell?
 * 
 * counter2 uses the callback because it references a variable that's not within the function's local scope
 * 
 * 3. In what scenario would the counter1 code be preferable? In what scenario would counter2 be better? 
 *
 * counter1 would be preferable if there is no need to rewference the function or variable at a later time, counter2 is preferable if you need to reference back to that function and variable later in your program.
*/
